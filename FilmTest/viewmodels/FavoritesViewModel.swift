...
//  FilmTest
//
//  Created by Kirill Mazaev on 15.05.2019.
...
class FavoritesViewModel: Messagable {
    ...
    fileprivate var dbManager: DBManager?
    fileprivate var notificationToken: NotificationToken?
    
    let sortedMovies = MutableObservableArray<Movie>([])
    var errorMessages = PassthroughSubject<String, Never>()
    let sortingType = Observable<Int>(0)
    ...
    fileprivate func fetchMovies() {
        ...
        if let movieResults = dbManager.fetch() {
            notificationToken = movieResults.observe (
    ...
    fileprivate func insertSortedMovies() {
        if (!sortedMovies.isEmpty) { sortedMovies.removeAll() }
        sortedMovies.insert(contentsOf: sortingFavoritesByType(), at: 0)
    }
